<template>
 ...
 <el-row class="align-center" style="margin-left: 28%;">
            <span style="font-size:20px">导出全免</span>
        </el-row>
        </br></br></br>
            <div style="margin-left:32%" >
                <el-form :model="freeReduceExport" ref="freeReduceExport" :rules="exportRules">
                     <el-form-item v-show=false prop="reduce"  label="优惠额度:" label-width="100px">
...
<script>
import { path,server,carditems,checkPhone,checkNumber,dtypelist,cardtypeitems,otypelist,accountitems,belongitems,settleitems,percision } from '../api/api';
...
//               console.log(vm.code+"1111111111111111111111111111111")
    ...
    exportReduce(){
        ...
        vm.type = 4;
         ...
         vm.$refs.freeReduceExport.validate((valid) => {
            if (valid) {
                vm.loading = true;
                //var tempwindow=window.open();
                 vm.$axios.post(path+"/shopticket/exportcode?shop_id="+sessionStorage.getItem('shopid')+"&number="+vm.freeReduceExport.number+"&type="+vm.type+"&reduce=1"+"&free_limit_times="+vm.free_limit_times+"&time_range="+vm.freeReduceExport.freeLimit,{
                    ...
                    //tempwindow.location = path + "/shopticket/export?codelist="+ret.codeList.toString()
