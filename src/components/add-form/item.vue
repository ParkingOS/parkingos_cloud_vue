<template>
    <el-form-item :rules="Rules" :label="item.label" :prop="item.prop" :class="{'block':item.block}">
        ...
        <input-number v-else-if="item.type==='number'" v-bind="$attrs" v-on="$listeners" :min="item.min" :max="item.max" :decimal1="item.decimal1" :append="item.append" :prepend="item.prepend" :disabled="item.disabled" :size="item.size" ></input-number>
        ...
        <el-select v-else-if="item.type==='select'" v-bind="$attrs" v-on="$listeners" :multiple="item.multiple" :disabled="item.disabled" :multiple-limit="item.multipleLimit" :size="item.size" :style="{width:'250px'}">
